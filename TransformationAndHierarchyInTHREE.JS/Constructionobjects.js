 ...
 * @property {number} [size=1]
 */
export class GrLoader extends GrObject {
  /**
   * @param {LoaderProperties} params
   */
  constructor(params = {}) {

    let loader = new T.Group();
    let exSettings = {
      steps: 2,
      depth: 1.5,
      bevelEnabled: true,
    ...
    // draw the body of the mini-loader
    let bodyCurve = new T.Shape();
    bodyCurve.moveTo(-1, 0);
    bodyCurve.lineTo(-1, 2.5);
    bodyCurve.lineTo(0.5, 2.5);
    bodyCurve.lineTo(1, 1.5);
    bodyCurve.lineTo(2, 1.5);
    bodyCurve.lineTo(2, 0);
    bodyCurve.lineTo(-1, 0);
    let bodyG = new T.ExtrudeBufferGeometry(bodyCurve, exSettings);
    let bodyM = new T.MeshStandardMaterial({ color: "orange", metalness: 0.6, roughness: 1 });
    let body = new T.Mesh(bodyG, bodyM);
    loader.add(body);
    body.position.setY(0.9);
    loader.rotateY(0.5);

    // draw the wheels
    let ring1G = new T.TorusBufferGeometry(0.6, 0.3, 8, 16);
    let r1m = new T.MeshStandardMaterial({ color: "gray", metalness: 0.5, roughness: 0.8 });
    let ring1 = new T.Mesh(ring1G, r1m);
    loader.add(ring1);
    ring1.position.setY(0.8);
    ring1.translateZ(1.7);
    ring1.translateX(0.5);

    let ring2G = new T.TorusBufferGeometry(0.6, 0.3, 8, 16);
    let r2m = new T.MeshStandardMaterial({ color: "gray", metalness: 0.5, roughness: 0.8 });
    let ring2 = new T.Mesh(ring2G, r2m);
    loader.add(ring2);
    ring2.position.setY(0.8);
    ring2.translateZ(-0.2);
    ring2.translateX(0.5);

    let forearm = new T.Group();
    let exSettings1 = {
      steps: 2,
      depth: 1.3,
      bevelEnabled: true,
    ...
    loader.add(forearm);
    forearm_curve.moveTo(-1.5, 0);
    forearm_curve.lineTo(-1.5, 0.2);
    forearm_curve.lineTo(0.2, 0.15);
    forearm_curve.lineTo(0.15, 0);
    forearm_curve.lineTo(-1.5, 0);
    let forearm_geom = new T.ExtrudeGeometry(forearm_curve, exSettings1);
    let arm_mat = new T.MeshStandardMaterial({ color: "white", metalness: 1, roughness: 1 });
    let frontArm = new T.Mesh(forearm_geom, arm_mat);
    forearm.add(frontArm);
    //forearm.translateZ(4);
    forearm.position.set(3.2, 1.3, 0.1);
    // forearm.rotateZ(-0.2);

    let bucket_group = new T.Group();
    forearm.add(bucket_group);
    //bucket_group.position.set(-1.4, 0, 0);
    let bucket_curve = new T.Shape();
    bucket_curve.moveTo(-0.25, -0.9);
    bucket_curve.lineTo(-0.5, -0.5);
    bucket_curve.lineTo(-0.4, -0.3);
    bucket_curve.lineTo(-0.3, -0.2);
    bucket_curve.lineTo(-0.15, 0);
    bucket_curve.lineTo(0.3, 0);
    bucket_curve.lineTo(0.05, -0.2);
    bucket_curve.lineTo(0.5, -1);
    bucket_curve.lineTo(-0.25, -0.9);
    let bucket_geom = new T.ExtrudeGeometry(bucket_curve, exSettings1);
    let bm = new T.MeshStandardMaterial({ color: "white", metalness: 1, roughness: 0.8 });
    let bucket = new T.Mesh(bucket_geom, bm);
    bucket_group.add(bucket);
    bucket.translateY(-0.2);
    bucket.translateX(0);
    bucket.rotateZ(1);

    // front glass
    let glassG = new T.BoxBufferGeometry(0.1, 0.6, 1);
    let gm = new T.MeshStandardMaterial({ color: "lightpink", metalness: 0, roughness: 1 });
    let glass = new T.Mesh(glassG, gm);
    glass.position.setY(3);
    glass.translateX(0.8);
    glass.translateZ(0.7);
    glass.rotateZ(0.4);
    loader.add(glass);

    super(`Loader-${loaderObCtr++}`, loader, [
      ["x", -10, 10, 0],
      ["z", -10, 10, 0],
      ["theta", 0, 360, 0],
      ["arm rotate", 0, 50, 45],
    ...
    this.whole_ob = loader;
    this.frontarm = forearm;
    this.bucket = bucket_group;
    ...
    this.whole_ob.position.z = params.z ? Number(params.z) : 0;
    let scale = params.size ? Number(params.size) : 1;
    loader.scale.set(scale, scale, scale);
  ...
  update(paramValues) {
    ...
    this.whole_ob.position.z = paramValues[1];
    this.whole_ob.rotation.y = degreesToRadians(paramValues[2]);
    this.frontarm.rotation.z = -degreesToRadians(paramValues[3]) + Math.PI / 12;
...
export class GrDumptruck extends GrObject {
  ...
  constructor(params = {}) {
    ...
    // draw the body of the dumptruck
    let bodyCurve = new T.Shape();
    bodyCurve.moveTo(-1, 0);
    bodyCurve.lineTo(-1, 2.5);
    bodyCurve.lineTo(0.5, 2.5);
    ...
    body.translateZ(0.1);
    dumptruck.translateZ(9);
    dumptruck.translateX(2);
    dumptruck.rotateY(-3.5);

    //draw the back of dumptruck
    let bodyBack = new T.BoxBufferGeometry(3.6, 0.4, 1.5);
    let bm = new T.MeshStandardMaterial({ color: "gray", metalness: 0.3, roughness: 1 });
    let back = new T.Mesh(bodyBack, bm);
    back.position.setY(1.3);
    back.translateX(-2.9);
    back.translateZ(0.8);
    back.rotateY(0);
    dumptruck.add(back);
    ...
    // draw the dump box
    let dumpBox = new T.Group();
    dumptruck.add(dumpBox);
    let boxCurve = new T.Shape();
    boxCurve.moveTo(-1, 0);
    boxCurve.lineTo(-1, 2.3);
    boxCurve.lineTo(-2, 2.3);
    boxCurve.lineTo(-2, 2.5);
    boxCurve.lineTo(-0.6, 2.5);
    boxCurve.lineTo(-0.3, 1.75);
    boxCurve.lineTo(3, 1.75);
    boxCurve.lineTo(2.5, 0);
    bodyCurve.lineTo(-1, 0);
    let boxG = new T.ExtrudeBufferGeometry(boxCurve, exSettings);
    let boxM = new T.MeshStandardMaterial({ color: "gray", metalness: 0.3, roughness: 1 });
    let box = new T.Mesh(boxG, boxM);
    dumpBox.add(box);
    dumpBox.position.set(-2.2, 1.5, 1.35);
    dumpBox.rotateY(-3.13);
    ...
    dumptruck.add(ring2);
    ring2.position.setY(0.8);
    ring2.translateZ(-0.1);
    ring2.translateX(0.5);

    let ring3G = new T.TorusBufferGeometry(0.6, 0.3, 8, 16);
    let r3m = new T.MeshStandardMaterial({ color: "gray", metalness: 0.5, roughness: 0.8 });
    let ring3 = new T.Mesh(ring3G, r3m);
    dumptruck.add(ring3);
    ring3.position.setY(0.8);
    ring3.translateZ(1.6);
    ring3.translateX(-3.5);

    let ring4G = new T.TorusBufferGeometry(0.6, 0.3, 8, 16);
    let r4m = new T.MeshStandardMaterial({ color: "gray", metalness: 0.5, roughness: 0.8 });
    let ring4 = new T.Mesh(ring4G, r4m);
    dumptruck.add(ring4);
    ring4.position.setY(0.8);
    ring4.translateZ(-0.1);
    ring4.translateX(-3.5);

    // front glass
    let glassG = new T.BoxBufferGeometry(0.1, 0.6, 1);
    let gm = new T.MeshStandardMaterial({ color: "pink", metalness: 0, roughness: 1 });
    let glass = new T.Mesh(glassG, gm);
    glass.position.setY(3);
    glass.translateX(0.85);
    glass.translateZ(0.7);
    glass.rotateZ(0.4);
    dumptruck.add(glass);
  ...
  update(paramValues) {
    ...
    this.whole_ob.position.z = paramValues[1];
    this.whole_ob.rotation.y = degreesToRadians(paramValues[2]);
    this.dumpbox.rotation.z = degreesToRadians(paramValues[3]) + Math.PI / 1.35;
  }
}
